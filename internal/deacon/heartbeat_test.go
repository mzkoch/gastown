...
func TestReadHeartbeat_FallbackLastPatrol_UsesModTimeForStale(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "deacon-test-*")
	if err != nil {
		t.Fatal(err)
	}
	...
	lastPatrol := time.Now().Add(-30 * time.Minute).UTC()
	data := []byte(`{"patrol_count":3,"last_patrol":"` + lastPatrol.Format(time.RFC3339Nano) + `","extraordinary_action":false}`)
	...
	if err := os.Chtimes(HeartbeatFile(tmpDir), modTime, modTime); err != nil {
		t.Fatal(err)
	}
	...
	if diff := hb.Timestamp.Sub(modTime); diff > time.Second || diff < -time.Second {
		t.Fatalf("fallback timestamp mismatch: got %s want %s", hb.Timestamp, modTime)
	}
}
