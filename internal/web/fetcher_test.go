...
func TestDetermineBeadsCIStatus(t *testing.T) {
issueOpen := &beads.Issue{Status: "open"}
issueClosed := &beads.Issue{Status: "closed"}
issueInProgress := &beads.Issue{Status: "in_progress"}
...
{
name:   "pass when merged",
issue:  issueClosed,
fields: &beads.MRFields{CloseReason: "merged"},
want:   "pass",
},
{
name:   "fail when conflict",
issue:  issueClosed,
fields: &beads.MRFields{CloseReason: "conflict"},
want:   "fail",
},
{
name:   "pending when open",
issue:  issueOpen,
fields: &beads.MRFields{},
want:   "pending",
},
{
name:   "pending when in progress",
...
for _, tt := range tests {
t.Run(tt.name, func(t *testing.T) {
got := determineBeadsCIStatus(tt.issue, tt.fields)
if got != tt.want {
t.Errorf("determineBeadsCIStatus() = %q, want %q", got, tt.want)
}
})
}
}

func TestDetermineBeadsMergeable(t *testing.T) {
blocked := &beads.Issue{Status: "open", BlockedBy: []string{"gt-task"}}
ready := &beads.Issue{Status: "open"}
inProgress := &beads.Issue{Status: "in_progress"}
closed := &beads.Issue{Status: "closed"}
...
{"ready when open", ready, "ready"},
{"conflict when blocked", blocked, "conflict"},
{"pending when in progress", inProgress, "pending"},
{"ready when closed", closed, "ready"},
}

for _, tt := range tests {
t.Run(tt.name, func(t *testing.T) {
got := determineBeadsMergeable(tt.issue)
if got != tt.want {
t.Errorf("determineBeadsMergeable() = %q, want %q", got, tt.want)
}
})
}
}
