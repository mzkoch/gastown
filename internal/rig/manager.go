...
`
	...
	// Settings are placed in parent directories (not inside git repos) so the runtime
	...
	for _, sr := range settingsRoles {
		runtimeConfig := config.ResolveRoleAgentConfig(sr.role, townRoot, rigPath)
...
func (m *Manager) createPatrolHooks(workspacePath string, runtimeConfig *config.RuntimeConfig) error {
	if runtimeConfig == nil || runtimeConfig.Hooks == nil {
		return nil
	}
	...
	switch runtimeConfig.Hooks.Provider {
	case "claude", "copilot":
		return runtime.EnsureSettingsForRole(workspacePath, "witness", runtimeConfig)
	default:
		return nil
	}
}
